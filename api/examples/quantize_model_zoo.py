...
class ModelZooBridge:
    ...
    def __init__(self, model_name, in_graph, data_location, models_zoo_path,
                 models_source_dir=None, models_config_file=None):
        ...
        if os.path.exists(self.models_config_file):
            with open(self.models_config_file, 'r') as config:
                ...
                for params in config_object:
                    if params[ModelZooBridge.MODEL_NAME] not in self.supported_model_list:
                        self.supported_model_list.append(params[ModelZooBridge.MODEL_NAME])
                    if self.model_name != params[ModelZooBridge.MODEL_NAME]:
                        continue
                    for key in params.keys():
                        self.model_param_dict[key] = params[key]
                        print("Model Config: %s:%s" % (key, self.model_param_dict[key]))
        else:
            print("Warning: File {} does not exist.".format(self.models_config_file))
        print("Model Config: Supported models - %s" % self.supported_model_list)

    def _inference_calib_cmd(self):
        launch_benchmark_params = self.model_param_dict[ModelZooBridge.LAUNCH_BENCHMARK_PARAMS]
        inference_calib_cmd = 'python ' + os.path.join(self.models_zoo_path,
                                                       launch_benchmark_params[ModelZooBridge.LAUNCH_BENCHMARK_SCRIPT])
        inference_calib_cmd += ' ' + ' '.join(launch_benchmark_params[ModelZooBridge.LAUNCH_BENCHMARK_CMD])
        inference_calib_cmd += ' ' + launch_benchmark_params[ModelZooBridge.DATA_LOCATION].format(self.data_location)
        inference_calib_cmd += ' ' + launch_benchmark_params[ModelZooBridge.IN_GRAPH]
    ...
    def _quantize_params_dict(self):
        params = self.model_param_dict[ModelZooBridge.QUANTIZE_GRAPH_CONVERTER_PARAMS]
        return params
...
def main(_):
    ...
    model = ModelZooBridge(args.model,
                           args.in_graph,
                           args.data_location,
                           args.models_zoo_location,
                           args.models_source_dir,
                           config_file_path)
    ...
    if ModelZooBridge.EXCLUDED_NODE_LIST in graph_converter_params.keys():
        excluded_nodes = graph_converter_params[ModelZooBridge.EXCLUDED_NODE_LIST]
    ...
    qt = converter.GraphConverter(args.in_graph,
                                  args.out_graph,
                                  inputs,
                                  outputs,
                                  excluded_ops,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', type=str, default=None, help='The model name to run quantization.')
    parser.add_argument('--in_graph', type=str, default=None, required=required_arg,
                        help='The path to input original fp32 frozen graph.')
    parser.add_argument('--out_graph', type=str, default=None,
                        help='The path to generated output int8 frozen graph.')
    parser.add_argument('--data_location', type=str, default=None, help='The path to dataset with tfrecord format.')
    parser.add_argument('--models_zoo_location', type=str, default=None,
                        help='Specify the root path to Model Zoo for Intel® Architecture'
                             ' to run the model from Model Zoo.')
    parser.add_argument('--models_source_dir', type=str, default=None,
                        help='Specify the path to tensorflow models which are only required by few models in Model Zoo.'
                             ' This argument can only be used in conjunction with a --models_zoo_location.')
    parser.add_argument("--debug", type=bool, default=False, help="Launches debug mode with rich debug information.")
