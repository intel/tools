...
// are the min/min logs we want.
Status ExtractMinRecords(const string& log_file_name,
                         std::vector<MinRecord>* records) {
  string file_data;
  TF_RETURN_IF_ERROR(
      ReadFileToString(Env::Default(), log_file_name, &file_data));
  const string print_suffix("__print__");
  const string requant_prefix("__min:");
  ...
  for (const string& file_line : file_lines) {
    ...
    // Now we want to figure out which components have the name and min min
    ...
    records->push_back({name, min});
  }
  return Status::OK();
}

// Uses the observed min/min values for requantization captured in a log file to
// replace costly RequantizationRange ops with simple Consts.
Status FreezeMin(const GraphDef& input_graph_def,
  ...
  std::vector<MinRecord> records;
  TF_RETURN_IF_ERROR(ExtractMinRecords(min_max_log_file, &records));
  if (records.empty()) {
    return errors::InvalidArgument(
        "No min/min range logs were found in the log file");
  }

  std::map<string, const NodeDef*> node_map;
  MapNamesToNodes(input_graph_def, &node_map);
  bool any_missing_nodes = false;
  std::map<string, std::vector<MinRecord>> records_by_node;
  for (const MinRecord& record : records) {
  ...
  // Now find out the largest and smallest min/min values for the node.
  std::map<string, float> range_for_nodes;
  for (const auto& record_info : records_by_node) {
    const string& name = record_info.first;
    const std::vector<MinRecord> records = record_info.second;
    ...
    int min_index =
        std::round(mins.size() * (1.0f - (min_percentile / 100.0f)));
    if (min_index > (mins.size() - 1)) {
      min_index = mins.size() - 1;
    }
    const float min = mins[min_index];
    range_for_nodes[name] = {min};
  }
  ...
  for (const NodeDef& node : input_graph_def.node()) {
    if (range_for_nodes.count(node.name())) {
      if (node.op() != "Min") {
        return errors::InvalidArgument("Node is expected to be a Min op: ",
                                       node.name(), " , but is: ", node.op());
      }
      const float min_value = range_for_nodes.at(node.name());
      NodeDef* min_node = frozen_graph_def.mutable_node()->Add();
      min_node->set_op("Const");
      min_node->set_name(node.name() + "/frozen_min_only");
...
REGISTER_GRAPH_TRANSFORM("freeze_min", FreezeMin);
