...
class FuseQuantizedMulAndRequantize(GraphTransformBase):
    ...
    def generate_output_graph(self, input_graph_def, input_node_map, fuse_op_list):
        ...
        for index, node in enumerate(input_graph_def.node):
            ...
            if index in fuse_op_list:
                ...
                if input_node.op == 'QuantizeV2':
                    ...
                    min_input_node = input_node_map[self.get_node_name_from_input(input_node.input[1])]
                    max_input_node = input_node_map[self.get_node_name_from_input(input_node.input[2])]
                    ...
                    min_input_value = (tensor_util.MakeNdarray(min_input_node.attr['value'].tensor))
                    max_input_value = (tensor_util.MakeNdarray(max_input_node.attr['value'].tensor))
                    min_filter_value = (tensor_util.MakeNdarray(min_filter_node.attr['value'].tensor))
                    max_filter_value = (tensor_util.MakeNdarray(max_filter_node.attr['value'].tensor))

                    weights_tensor = tensor_util.MakeNdarray(input_node_map[weights_node_name].attr['value'].tensor)
                    bias_tensor = tensor_util.MakeNdarray(input_node_map[bias_node_name].attr['value'].tensor)
                    bias_scale = 255.0 * 127.0 / (
                        max(abs(max_input_value), abs(min_input_value)) *
                        max(abs(max_filter_value), abs(min_filter_value)))
                    QaAmin = 255 * min_input_value / (max_input_value - min_input_value)
                    ...
                    bias_node = self.check_node_existence(output_graph_def, bias_node_name)
                    if not bias_node:
                    ...
                    output_graph_def.node.extend([new_node, frozen_max_node, frozen_min_node])
                    ...
                    weights_node_name = node.input[1]
                    bias_node_name = node.input[2]
                    min_input_node = input_node_map[self.get_node_name_from_input(input_node.input[3])]
                    max_input_node = input_node_map[self.get_node_name_from_input(input_node.input[4])]
                    requantize_node = input_graph_def.node[index + 3]
                    frozen_max_node = input_graph_def.node[index + 2]
                    frozen_min_node = input_graph_def.node[index + 1]
                    skip_list.append(index + 1)
                    ...
                    bias_node.attr['dtype'].CopyFrom(attr_value_pb2.AttrValue(type=qint32_type))
                    bias_node.attr['value'].CopyFrom(attr_value_pb2.AttrValue(
                        tensor=tensor_util.make_tensor_proto(bias_int32, dtypes.int32, bias_tensor.shape)))
                    bias_node.attr['value'].tensor.dtype = qint32_type
                    new_node.attr["Tbias"].CopyFrom(attr_value_pb2.AttrValue(type=qint32_type))
                    new_node.attr["Toutput"].CopyFrom(attr_value_pb2.AttrValue(type=uint8_type))
    ...
    def do_transformation(self):
        ...
        """
        input_node_map, _, node_name_list = self.parse_input_graph(self.input_graph)
        fuse_op_list = self.get_fuse_index(input_node_map, node_name_list)
        return self.generate_output_graph(self.input_graph, input_node_map, fuse_op_list)
